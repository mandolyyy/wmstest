...
A small Warehouse Management System built with **React** (frontend), **Express** (backend API), and **SQLite** — using Node's built-in `node:sqlite` module (available in Node 22.5+), so there's no native module to compile and nothing that can mismatch your OS/architecture.

> **Requires Node.js 22.5 or newer.** Check with `node -v`. You'll see a one-line `ExperimentalWarning: SQLite is an experimental feature` in the terminal when the server starts — that's expected and harmless.

## Features

1. **Inbound** — log a received item's weight, dimensions, timestamp and operator (user ID). The system automatically assigns it to the smallest empty bin that can fit its weight and dimensions.
2. **Storage setting** — define the physical hierarchy **Zone → Aisle → Rack → Shelf → Bin**, with each bin's ID, max weight and max dimensions.
3. **Outbound** — create a pre-request for a stored item (item ID, datetime, user ID), then confirm outbound to ship it and free its bin.
4. **Reporting** — every action (inbound received, bin assigned, outbound requested, outbound completed, etc.) is logged with a timestamp and user ID. Look up the full process trail for any item ID, or browse/filter the global log feed.
